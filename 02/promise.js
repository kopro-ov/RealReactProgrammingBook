...
/*
(1) 일반적으로 new 키워드를 사용해서 프로미스를 생성한다.
이 방법으로 생성된 프로미스는 대기 중 상태가 된다.
생성자에 입력되는 함수는 resolve와 reject라는 콜백 함수로 매개변수로 갖는다.
비동기로 어떤 작업을 수행 후 성공했을 때 resolve를 호출하고 실패했을 때 reject를 호출하면 된다.
resolve를 호출하면 p1 객체는 이행됨 상태가 된다.
반대로 reject를 호출하면 거부됨 상태가 된다.
만약 생성자에 입력된 함수 안에서 예외가 발생하면 거부됨 상태가 된다.
new 키워드를 사용해서 프로미스를 생성하는 순간 생성자의 입력함수가 실행된다.
만약 API 요청을 보내는 비동기 코드가 있다면 프로미스가 생성되는 순간 요청을 보낸다.

(2) new키워드를 사용하지 않고 Promise.reject를 호출하면 거부됨 상태인 프로미스가 된다.
(3) Promise.resolve를 호출해도 프로미스가 생성된다. 만약 입력값이 프로미스였다면 그 객체가 그대로 반환되고,
프로미스가 아니라면 이행됨 상태인 프로미스가 반환된다.
*/
...
/*
(1) 프로미스가 아닌 인수와 함께 Promise.resolve함수를 호출하면 그 값 그대로 이행됨 상태인 프로미스가 반환된다. (Promise {<fulfilled>: 123})
...
//데이터 요청의 성공, 실패 여부와 상관없이 서버에 로그를 보낼 때 finally 메서드를 사용한 코드. finally는 새로운 프로미스를 생성하지 않는다.
...
/*
입력된 모든 프로미스가 처리됨 상태가 되어야 마찬가지로 처리됨 상태가 된다.
만약 하나라도 거부됨 상태가 된다면 Promise.all 함수가 반환하는 프로미스도 거부됨 상태가 된다.
*/

//가장 빨리 처리된 프로미스 가져오기: Promise.race
//Promise.race는 여러 개의 프로미스 중 가장 빨리 처리된 프로미스를 반환하는 함수
Promise.race([
  requestData(),
...
/*
(1) then메서드는 기존 객체를 수정하지 않고 새로운 프로미스를 반환한다.
...
/*
만약 (1)에서 result1 변수를 참조해야 한다면 어떻게 해야 할까?
