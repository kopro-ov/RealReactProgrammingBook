...
- 배열 비구조화
*/
//전개 연산자는 배열이나 객체의 모든 속성을 푸어놓을 때 사용하는 문법이다.
...
만약 네개의 변수를 사용하면 값은 동적으로 전달할 수 있지만 매개변수 개수는 항상 네 개로 고정이다.

(2) 전개 연산자를 사용하면 동적으로 함수의 매개변수를 전달할 수 있다.
*/

//전개 연산자를 사용하지 않고 동적으로 함수의 매개변수를 전달하는 다른 방법
...
const { age, name } = obj; //객체 비구조화에서는 중괄호를 사용한다. 배열 비구조화에서는 배열의 순서가 중요했지만 객체 비구조화에서 순서는 무의미하다.
console.log(age);
console.log(name);
...
const obj = {age: undefined, name: null, grade: 'A'}; //속성값이 null이면 기본값은 들어가지 않는다.
